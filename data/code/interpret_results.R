...
option_list <- list(
  ...
  make_option(c("-a", "--info_file_r1"), type="character", default="info1.csv",
    help="Name of file containing region 1 information (Drug-resistance Mutation)"),
  make_option(c("-b", "--info_file_r2"), type="character", default="info2.csv",
    help="Name of file containing region 2 information (Mediator)"),
  make_option(c("-o", "--out_file"), type="character", default="results_comp_mut.csv",
    help="Name of file to output significant results"),
  make_option(c("-c", "--chi_only"), type="character", default="chi_only.csv",
    help="significant chi results only file") 
    )
# Parse Options
...
interpret_results <- function(results_file, info_file_r1, info_file_r2, out_file, chi_file){
  ...
  colnames(info_r1) <- paste("snp1", colnames(info_r1), sep = "_")
  colnames(info_r2) <- paste("snp2", colnames(info_r2), sep = "_")
  colnames(info_r1) <-  gsub("snp1_snp", "snp1", colnames(info_r1))
  colnames(info_r2) <- gsub("snp2_snp", "snp2", colnames(info_r1))
  ...
  sig_rows <- nrow(results %>% filter((ab_ci_l>0 & ab_ci_u>0)|(ab_ci_l<0 & ab_ci_u<0)))
  results <- results[!is.na(results$ab_ci_l), ]
  if (sig_rows >0){
  ...
  nonsig_rows <- nrow(results %>% filter((ab_ci_l<0 & ab_ci_u>0)))
  if (nonsig_rows>0){
