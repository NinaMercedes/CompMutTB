...
option_list <- list(
  ...
  make_option(c("-s", "--sample_names"), type="character", default="character",
    help="Sample names txt file")   
    )
# Parse Options
...
snp_pheno_test <- function(geno, meta, drug, sample_names){
  ...
  #rownames(geno) <- s_names$V1
  geno <- geno[rownames(geno) %in% metadata$id,]
  metadata <- metadata[metadata$id %in% rownames(geno),]
  geno <- geno[order(match(rownames(geno), metadata$id)), , drop = FALSE]
  phen_tables <- list()
...
if(opt$assoc_test==TRUE){
  ...
  phen_tables_a <- snp_pheno_test(opt$geno_a, opt$meta, opt$drug, opt$sample_names)
  phen_tables_b <- snp_pheno_test(opt$geno_b, opt$meta, opt$drug, opt$sample_names)
